...
interface Props {
	data: Product;
	openActions: { id: number | null; isOpen: boolean };
	activeIndex: number | null;
	toggleOpenActions: (id: number | null, isOpen: boolean) => void;
	togglePreview: (
		id: number | null,
		action: Action | null,
...
export default function TableRow(props: Props) {
	const { data, openActions, activeIndex, toggleOpenActions, togglePreview } =
		props;

	function checkIsOpen(id: number | null, isOpen: boolean) {
		if (id === data.id && isOpen) return true;
	}
	...
	return (
		<tr key={data.id} className='border-b dark:border-gray-700'>
			...
			<td className='px-4 py-3 max-w-[12rem] truncate'>{data.description}</td>
			...
			<td className='px-4 py-3 flex items-center justify-end'>
				<button
					...
					type='button'
					onClick={() => toggleOpenActions(data.id, openActions.isOpen)}
				>
					<svg
						className='w-5 h-5'
						aria-hidden='true'
				...
				<div
					...
					className={
						checkIsOpen(openActions.id, openActions.isOpen)
					...
					>
						<li>
							<button
								...
								onClick={() => togglePreview(data.id, Action.EDIT, data)}
							>
								<svg
						...
						<li>
							<button
								...
								className='flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200'
								onClick={() => togglePreview(data.id, Action.PREVIEW, data)}
							>
								<svg
									className='w-4 h-4 mr-2'
									xmlns='http://www.w3.org/2000/svg'
						...
						<li>
							<button
								...
								onClick={() => togglePreview(data.id, Action.DELETE, data)}
							>
								<svg
									className='w-4 h-4 mr-2'
									viewBox='0 0 14 15'
